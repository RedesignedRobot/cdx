...
State has one owner. A Claude session is a delivery cursor, not an owner: the newest session that ran `cdx brief` (session start, resume, compaction) and polled within 30 s is the head and receives events. Older open sessions receive only messages addressed to their full session id. A new session starts at the newest event; history is the brief's job. `cdx brief` prints running lanes, the five newest finished lanes awaiting attention, open questions, and recent jobs, and prints nothing when the same text ran within 10 minutes.
