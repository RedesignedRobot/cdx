...
`usage --line` prints status line rows. A `codex` row shows each Codex account's weekly window as used percent and time to reset (`codex-1 52% ↻3d3h`). A `gemini` row shows Gemini's weekly and five-hour windows. It reads only the stored snapshots, never probes, and takes about 30ms, so a status line can call it on every render. A window past its reset shows `0%`, and an account with no reading shows `?`. It prints 256-color ANSI into a pipe, because status lines read it that way, unless `NO_COLOR` is set. Each row starts with a colored chip, steel blue for `codex` and periwinkle for `gemini`. Healthy percentages are soft green, turning amber at 75% and red at 95%, and reset timers are soft blue, the same scheme as `cca line`.
