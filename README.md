...
The plugin monitor runs `cdx watch` with no argument. It reads `CLAUDE_CODE_SESSION_ID` and `CLAUDE_PID` from the monitor environment and refuses to run without them. No head setup call is needed. A persisted lease allows one live watcher per full session id, tied to that Claude process. A watcher that finds a live lease stands by and takes over when the holder exits, so a reload that starts the new monitor before the old one stops still ends with one watcher resuming the saved wake cursor.
