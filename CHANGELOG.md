...
- `doctor` checks every configured Codex home against the primary home for matching `AGENTS.md`, matching MCP server definitions, and a valid `hooks.json`. For Codex homes, `--fix` copies directives only and reports manual config or hook repairs.
