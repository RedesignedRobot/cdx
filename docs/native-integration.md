...
- The head is the session, among those that polled within 30 seconds, that most recently drove cdx: spawn, resume, send, review, consult, reply, land, or `cdx brief --head`. With no active driver, the longest-running active session is the head. A session that only started and polled never takes the wakes.
